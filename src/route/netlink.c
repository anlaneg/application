...
nl_msg_nlmsgerr(const struct ofpbuf *msg, int *errorp)
{
    if (nl_msg_nlmsghdr(msg)->nlmsg_type == NLMSG_ERROR) {
        ...
        int code = EPROTO;
        if (!err) {
            VLOG_ERR("received invalid nlmsgerr (%d bytes < %ld)",
...
{
    assert(nla->nla_len >= NLA_HDRLEN);//属性必须大于等于最小长度
...
{
    assert(nla->nla_len > NLA_HDRLEN);
    assert(memchr(nl_attr_get(nla), '\0', nla->nla_len - NLA_HDRLEN));
...
nl_attr_validate(const struct nlattr *nla, const struct nl_policy *policy)
{
    ...
    if (len < min_len || len > max_len) {
        VLOG_DBG("attr %"PRIu16" length %ld not in "
                    "allowed range %ld...%ld", type, len, min_len, max_len);
        return false;
    }
    ...
    if (policy->type == NL_A_STRING) {
        if (((char *) nla)[nla->nla_len - 1]) {
            VLOG_DBG( "attr %"PRIu16" lacks null at end", type);
            return false;
        }
        if (memchr(nla + 1, '\0', len - 1) != NULL) {
            VLOG_DBG("attr %"PRIu16" has bad length", type);
            return false;
        }
    }
                ...
                struct nlattr *attrs[], size_t n_attrs)
{
                      ...
                      msg->size - nla_offset)
    {
        ...
        if (type < n_attrs && policy[type].type != NL_A_NO_ATTR) {
            ...
            if (attrs[type]) {
                VLOG_DBG( "duplicate attr %"PRIu16, type);
            }
            attrs[type] = nla;
        }
    }
    if (left) {
        VLOG_DBG("attributes followed by garbage");
        return false;
    }
    ...
    for (i = 0; i < n_attrs; i++) {
        ...
        if (!e->optional && e->type != NL_A_NO_ATTR && !attrs[i]) {
            VLOG_DBG("required attr %ld missing", i);
            return false;
        }
    }
    return true;
